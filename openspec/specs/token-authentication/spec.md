...
Authenticating API requests via `Authorization: Bearer <token>`, mapping a token to a `SecurityIdentity` with the snapshotted role, and how token auth coexists with the existing OIDC flow.
